 ...
 *않아 {@link SchoolCd}/{@link JobCd}/{@link SbizCd}처럼 라벨 enum으로 만들지 않았다 — 틀린 라벨을 매칭 로직에 하드코딩하는 위험을 피하기
 * 위함. 매칭에는 정규화된 코드값만 쓰고, UI 라벨이 필요해지면 실제 코드정의서/API로 라벨을 검증한 뒤 enum으로 승격한다.
 */
